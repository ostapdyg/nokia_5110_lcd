...
void lcd_send(uint8_t bytes, uint8_t is_data)
{
	HAL_GPIO_WritePin(LCD_PORT, LCD_DC, is_data);

	HAL_GPIO_WritePin(LCD_PORT, LCD_CE, 0);

	for (int i = 0; i < 8; i++)
	{
		HAL_GPIO_WritePin(LCD_PORT, LCD_DIN, !!((bytes << i) & 0x80)); // Most significant bit first
		HAL_GPIO_WritePin(LCD_PORT, LCD_CLK, 1);
		HAL_GPIO_WritePin(LCD_PORT, LCD_CLK, 0);
	}

	HAL_GPIO_WritePin(LCD_PORT, LCD_CE, 1);
}
...
void lcd_init(void)
{
	...
	lcd_send_cmd(0x21); // extended instruction set control (H=1)

	lcd_send_cmd(0x13); // LCD bias mode 1:48

	lcd_send_cmd(0x06); // Set temperature coefficient

	lcd_send_cmd(0xC2); //Default VOP (3.06 + 66 * 0.06 = 7V)

	lcd_send_cmd(0x20); //Standard Commands mode, powered down

	lcd_send_cmd(0x09); // all display segments on
...
void lcd_render_all()
{
	...
	for (int i = 0; i < 504; i++)
		lcd_send_data(nokia_lcd.buffer[i]);
}
